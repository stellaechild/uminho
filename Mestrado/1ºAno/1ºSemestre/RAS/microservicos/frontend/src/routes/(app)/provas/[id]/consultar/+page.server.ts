...
import { TipoPerguntas, UserTypes, type ProvaEstudante } from '$lib/types';
export const load: PageServerLoad = async ({ params, locals }) => {
	if (locals.user.type == UserTypes.NotAuthenticated)
		...
		throw error(403, {
			message: 'Não autorizado. Tens de ser estudante para aceder a esta página.',
			title: 'Proibido',
			redirect: '/'
		});
	//todo check if aluno hasnt realized prova
	//todo fetch prova
