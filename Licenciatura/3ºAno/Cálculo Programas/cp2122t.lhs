...
%
        |lcbr(
          outNEList . singl a = i1 a 
     )(
          outNEList . cons (a,b) = i2 (a,b)
     )|
%
...
\xymatrix@@C=2cm{
     |LTree A| 
     && 
     |A + (LTree A >< LTree A)| \ar[ll]_-{inLTree}
\\
     |A|^* \ar[r]^-{|outNEList|} \ar[u]^-{|list2LTree|} \ar@@/_1.5pc/[rr]_-{|g_list2LTree|}
&
     |A + (A >< A|^*|)| \ar[r]^-{|id + divLista . cons|}
&
     |A + (A|^* | >< A|^*|)| \ar[u]_-{|id + list2LTree|^2} 
}
...
Gene do anamorfismo:
\begin{code}
g_list2LTree = (id -|- divLista . cons) . outNEList     
divLista l = ((take ((length l) `div`  2) l),(drop ((length l) `div`  2) l))
\end{code}
...
g_lTree2MTree = either g1 g2
          where g1 = Unit . split Main.hash id
                g2 = uncurry Comp . split (Main.concHash . ((firsts . outFTree) >< (firsts . outFTree))) id
\end{code}
...
\just\equiv{ Fusão-+ (20) ; Absorção-+ (22)}
     |either (wc_w . nil) (wc_w . cons) = either (h1 . id) (h2 . (id >< split wc_w lookahead_sep))|
%
...
\just\equiv{ Igualdade Extensional (71) ; Def-comp (72); Def-const (74), Def-nil}
          ...
          wc_w (cons a) = h2 ((id >< split wc_w lookahead_sep) a)
     )|
%
\just\equiv{ |wc_w [] = 0| ; a := (a,b) ; Def-x (77)}
          |lcbr(
          0 = h1 a
     )(
          wc_w (cons (a,b)) = h2 (id a, (split wc_w lookahead_sep) b)
     )|
%
\just\equiv{ Def-id (73); Def-split (76); Def-cons}
          ...
          wc_w (a:b) = h2 (a, (wc_w b, lookahead_sep b))
     )|
%
\just\equiv{ Propriedade reflexiva da igualdade ; |wc_w (a:b) = if not (sp a) && lookahead_sep b then wc_w b + 1 else wc_w b|}
        |lcbr(
          h1 a = 0
     )(
          h2 (a, (wc_w b, lookahead_sep b)) = if not (sp a) && lookahead_sep b then wc_w b + 1 else wc_w b
     )|
%
\just\equiv{Def-cond (78)}
        |lcbr(
          ...
          h2 (a, (wc_w b, lookahead_sep b)) = cond (not (sp a) && lookahead_sep b) (wc_w b + 1) (wc_w b)
     )|
%
\just\equiv{Def succ; |andOperator| ; |andOperator:=and| ; |lookahead_sep:=la| ; |wc_c:=wc| ; (Abreviaturas)}
        |lcbr(
          h1 a = 0
     )(
          h2 (a, (wc b, la b)) = cond (and (not (sp a), la b)) (succ (wc b)) (wc b)
     )|
%
\just\equiv{Def-comp (72)}
        |lcbr(
          ...
          h2 (a, (wc b, la b)) = cond (and ((not . sp) a, la b)) (succ (wc a)) (wc b)
     )|
%
\just\equiv{Def-proj (79)}
        |lcbr(
          ...
          h2 (a, (wc b, la b)) = cond (and ((not . sp) x, p2 (wc y, la y))) (succ (p1 (wc y, la y))) (p1 (wc y, la y))
     )|
%
\just\equiv{Def-x (77), Def-comp (72)}
        |lcbr(
          ...
          h2 (a, (wc b, la b)) = cond ((and . ((not . sp) >< p2)) (a,(wc b, la b))) ((succ . p1) (wc b, la b)) (p1 (wc b, la b))
     )|
%
\just\equiv{Def-x (79), Def-comp (72)}
        |lcbr(
          ...
          h2 (a, (wc b, la b)) = cond ((and . ((not . sp) >< p2)) (a,(wc b, la b))) ((succ . p1 . p2) (a,(wc b, la b))) (p1 . p2 (a,(wc b, la b)))
     )|
%
\just\equiv{Def-cond (78)}
        |lcbr(
          ...
          h2 (a, (wc b, la b)) = (cond (and . ((not . sp) >< p2)) (succ . p1 . p2) (p1 . p2)) (a,(wc b, la b)
     )|
\end{eqnarray}

Do último passo que determinou a versão \textit{pointwise} deduzimos que:
que $|h1|$ é $|zero|$ e $|h2|$ é a função
$|cond (andOperator . ((not . sp ) >< p2)) (succ . p1 . p2) (p1 . p2)|$.
...
%
\just\equiv{ Fusão-+ (20) ; Absorção-+ (22)}
     |either (lookahead_sep . nil) (lookahead_sep . cons) = either (k1 . id) (k2 . (id >< split wc_w lookahead_sep))|
%
\just\equiv{ Eq-+ (27); Natural-id (1)}
          |lcbr(
...
\just\equiv{ Igualdade Extensional (71); Def-comp (72); Def-const (74), Def nil}
          ...
          lookahead_sep (cons a) = k2 ((id >< split wc_w lookahead_sep) a)
     )|
%
\just\equiv{ |lookahead_sep [] = True| ; a := (a,b) ; Def-x (77)}
          |lcbr(
          True = k1 a
     )(
          lookahead_sep (cons (a,b)) = h2 (id a, (split wc_w lookahead_sep) b)
     )|
%
\just\equiv{ Def-id (73); Def-split (76) ; Def cons}
          |lcbr(
          ...
          lookahead_sep (a:b) = k2 (a, (wc_w b, lookahead_sep b))
     )|
%
\just\equiv{|lookahead_sep (aby) = sp a|}
          |lcbr(
          True = k1 a
     )(
          sp x = k2 (a, (wc_w b, lookahead_sep b))
     )|
%
\just\equiv{Propriedade reflexiva da igualdade}
        |lcbr(
          ...
          k2 (a, (wc_w b, lookahead_sep b)) = sp a
     )|
\end{eqnarray}

Assim, chegamos à conclusão que $|k1|$ é sempre $|const True|$ e $|k2|$ é dado pela função $|sp . p1|$.
\newpage
Utilizando a Lei da Troca, podemos obter a definição final do gene da função worker.
\begin{eqnarray*}
\start
     |cata (split (either h1 h2) (either k1 k2))|
%
\just\equiv{ Lei da troca (28) }
%
     |cata (either (split h1 k1) (split h2 k2))|
\end{eqnarray*}
\paragraph{}
...
\begin{code}
h1 = zero
h2 = cond (andOperator . ((not . sp) >< p2)) (succ . p1 . p2) (p1 . p2)
k1 = const True
k2 = sp . p1
...
\xymatrix@@C=2cm{
     Char^{*}  \ar[d]_-{worker}
     & 
     1 + (Char \times Char^{*}) \ar[d]^-{|recList(worker)|} \ar[l]_-{|in = either nil cons|}
\\
     (Integer,Bool)
     &
     1 + (Char \times (Integer,Bool))  \ar[l]_-{|[g1,g2]|} 
}
...
\\
     A^{*} \ar[r]^-{|outList|} \ar[u]^-{|pairL|} \ar@@/_1.5pc/[rr]_-{|g|}
&
     1 + (A \times A^{*}) \ar[r]^-{|id + pairHandler|}
&
     1 + ((A,A) \times A^{*}) \ar[u]_-{|recList(pairL)|} 
}

\paragraph{}
Para o gene do anamorfismo, começamos por criar uma composição das funções |outList| e a função que será responsável pela criação dos pares.
Definimos esta função como uma alternativa de um |id| com uma subfunção chamada |pairHandler|. Esta função começa por avaliar se a lista fornecida era singular,
pois assim gere apenas um par repetido (|singlePair|). Caso a lista fornecida contenha múltiplos elementos (|multiplePairs|), este vai gerando os pares com a cabeça da lista e a cabeça da cauda.
...
\par
Tivemos também a necessidade de definir duas funções auxiliares |emptyList| e |singlList|. Estas apenas avaliam uma dada lista, indicando se esta está
...
pairL = anaList g where
     ...
     pairHandler = cond (emptyList . p2) singlePair multiplePairs
     singlePair = split (split p1 p1) nil
     multiplePairs = split (split p1 (head . p2)) (cond (singlList . p2) nil p2)
...
constituintes da mesma: |divide and conquer|. A técnica de |divide|, neste caso em específico, refere-se à construção (anamorfismo) de pares de posições
a partir de uma lista de posições iniciais. Na segunda parte (|conquer|), é necessário consumir (catamorifismo) as mesmas posições, obtendo o mapa com todas 
as movimentações. 
\par
Como a primeira parte deste hilomorfismo já foi desenvolvida na alínea anterior (|pairL|), começamos por desenvolver a etapa de |conquer|, 
...
\par
Decidimos por isso começar por desenvolver uma função que, ao receber um par de posições e um mapa, efetuava a modificação correta neste (|updateCell|). 
Para esta função foi necessário desenvolver as funções auxiliares |updateElemList| e |getAtIndex|, sendo que a primeira atualiza um elemento numa dada lista,
...
as modificações utilizando o mapa que é originado da aplicação do functor recursivo, devido à tipagem deste ser (|Map -> Map|) quando nós apenas esperavamos Map.
\par
...
conseguimos utilizar o mapa "modificado" até aquele ponto. Apresentamos também uma função |updateMap| que faz todas modificações, obtendo o mapa pretendido,
...
\xymatrix@@C=2cm{
     Pos^{*} \ar[d]^-{|pairL|} \ar@@/_3pc/[dd]_-{|markMap m|} \ar[r]^-{g=|(either id pairHandler) . outList|}
     &
     1 + ((Pos,Pos) \times Pos^{*}) \ar[d]^-{|recList(pairL)|}
\\
     (Pos \times Pos)^{*} \ar[r]^-{outList} \ar[d]^-{|cataList (either (const id) f2) m|}
     & 
     1 + ((Pos \times Pos) \times (Pos \times Pos)^{*}) \ar[d]^-{|recList(cataList (either (const id) f2) m)|}
\\
     Map
     &
     1 + ((Pos \times Pos) \times Map)  \ar[l]_-{|either (const id) f2|} 
}
...
updateCell (po1@(x1,y1),po2) m = updateElemList y1 (updateElemList x1 (toCell po1 po2) (getAtIndex y1 m)) m

updateElemList ::  Int -> a -> [a] -> [a]
updateElemList n x l = uncurry (++) (((id >< curry cons x . drop 1) . (splitAt n)) l)
